...
class QueueRuntime {
  ...
  shellGroupAlive(shell) {
    if (process.platform !== 'win32' && shell.pgid) {
      try { process.kill(-shell.pgid, 0); return true; }
      catch (error) { return error && error.code !== 'ESRCH'; }
    }
    return !(shell.closed || shell.child.exitCode !== null && shell.child.exitCode !== undefined || shell.child.signalCode);
  }
